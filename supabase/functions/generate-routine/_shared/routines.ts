// Routine generation for the Supabase Edge Function (Deno runtime).
//
// keep in sync with lib/routines.ts + lib/surveys.ts + lib/types.ts
//
// This is a hand-ported, Deno-compatible copy of the Next.js generation logic.
// Edge Functions run on Deno and do not share the Next.js `lib/` build tree,
// so the prompt assembly, the SYSTEM_PROMPT, the fence-tolerant JSON parsing,
// and the strict schema validation are duplicated here verbatim. The SYSTEM
// PROMPT is the spec's coaching prompt; do not paraphrase it. Any change to
// prompt or validation on either side MUST be mirrored on the other.
...
// Minimal survey shape buildRoutinePrompt needs (camelCase domain type).
...
// Maps a persisted surveys row to the camelCase shape buildRoutinePrompt uses.
...
// caller can fail the job cleanly without ever persisting partial data.
